...
class DotBot:
    ...
    def generalize_learned(self):
        for xy in self.pos_exp_sensors.keys():
           self.what_i_learned[self.pos_exp_sensors[xy][1]] = self.direction\
								(xy, self.pos_exp_sensors[xy][0]) 
    ...
    def move_by_know(self):
        ...
        if s1to8 in self.what_i_learned.keys():
            if(self.what_i_learned[s1to8] == 'right'):
                self.move_right()
            if(self.what_i_learned[s1to8] == 'left'):
                self.move_left()
            if(self.what_i_learned[s1to8] == 'up'):
                self.move_up()
            if(self.what_i_learned[s1to8] == 'down'):
                self.move_down()
        else:
