...
class T3():
    ...
    def random_O(self):
        self.present()
        print "Playing an @ ..."
        if self.goodmove(self.row1):
            pick = self.bestchoice(self.row1)

        if self.goodmove(self.row2):
           pick = self.bestchoice(self.row2)

        if self.goodmove(self.row3):
           pick = self.bestchoice(self.row3)

        if self.goodmove(self.col1):
           pick = self.bestchoice(self.col1)

        if self.goodmove(self.col2):
           pick = self.bestchoice(self.col2)

        if self.goodmove(self.col3):
           pick = self.bestchoice(self.col3)

        if self.goodmove(self.dia1):
           pick = self.bestchoice(self.dia1)

        if self.goodmove(self.dia2):
           pick = self.bestchoice(self.dia2)
    ...
    def goodmove(self, arr):
        ...
        for x in arr:
            ...
            if(self.ttt[x] == 'X'):
                self.val = self.val + 1
            if(self.ttt[x] == '@'):
               self.o_val = self.o_val + 1

        if(self.val == 2 and not self.o_val == 1):
           return True

        return False

    def bestchoice(self, arr):
        if(self.val == 2 and self.o_val == 0):
            for i in arr:
                if not self.ttt[i] == 'X':
                    return self.ttt[i]
