...
class T3():
    ...
    def mini_max_X(self):
        ...
        for (nxtplX,nxtX) in next_boards_for_X:
            min_O = self.min_for_O_under_X(nxtX)
            mins_for_O.append((nxtplX, min_O))
        placeX = max(mins_for_O, key = lambda o: o[1])
        print "... X minimaxs with at %s" % placeX[0]
        return placeX[0]
    #-----------------------------------------------------
    ...
    def min_for_O_under_X(self,Xboard):
        places_for_O = []
        for o in Xboard.ttt.keys():#inverse of max for x under o
            if Xboard.ttt[o] == 0:
                places_for_O.append(o)
        maxeval = 10000
        for o in places_for_O:
            nxt = copy.deepcopy(self.ttt)
            nxt[o] = '@'
            nxtT3 = T3(nxt)#set up new board with new state
            nxteval = nxtT3.eval_ttt()#evaluate state
            if nxteval < maxeval:
                maxeval = nxteval
    ...
    def grab_winX(self):
        toplay = self.two_XO('X')
        if toplay != None:
            print "... X grabs a win at %s" % toplay
            return toplay
        return None
    #-----------------------------------------------------
